...
module aes_core(
    ...
    function [7:0] gf_mul2;
        input [7:0] in;
        gf_mul2 = (in[7]) ? ((in << 1) ^ 8'h1b) : (in << 1);
    endfunction

    // GF(2^8) multiplication by 3
    function [7:0] gf_mul3;
        input [7:0] in;
        gf_mul3 = gf_mul2(in) ^ in;
    endfunction
    ...
    function [127:0] inv_sub_bytes;
        ...
        begin
            for (i = 0; i < 16; i = i + 1) begin
                inv_sub_bytes[i*8 +: 8] = inv_sbox(in[i*8 +: 8]);
            end
        end
    endfunction
    ...
    function [127:0] shift_rows;
        ...
        begin
            shift_rows = {
                in[127:120], in[87:80], in[47:40], in[7:0],      // Row 0
                in[95:88], in[55:48], in[15:8], in[103:96],       // Row 1
                in[63:56], in[23:16], in[111:104], in[71:64],     // Row 2
                in[31:24], in[119:112], in[79:72], in[39:32]      // Row 3
            };
        end
    endfunction
    ...
    function [127:0] inv_shift_rows;
        ...
        begin
            inv_shift_rows = {
                in[127:120], in[23:16], in[47:40], in[71:64],     // Row 0
                in[95:88], in[119:112], in[15:8], in[39:32],      // Row 1
                in[63:56], in[87:80], in[111:104], in[7:0],       // Row 2
                in[31:24], in[55:48], in[79:72], in[103:96]       // Row 3
            };
        end
    endfunction
    ...
    function [127:0] mix_columns;
        ...
        begin
            for (i = 0; i < 4; i = i + 1) begin
                ...
                mix_columns[i*32 +: 8] = gf_mul2(a0) ^ gf_mul3(a1) ^ a2 ^ a3;
                mix_columns[i*32 + 8 +: 8] = a0 ^ gf_mul2(a1) ^ gf_mul3(a2) ^ a3;
                mix_columns[i*32 + 16 +: 8] = a0 ^ a1 ^ gf_mul2(a2) ^ gf_mul3(a3);
                mix_columns[i*32 + 24 +: 8] = gf_mul3(a0) ^ a1 ^ a2 ^ gf_mul2(a3);
            end
        end
    endfunction
    ...
    function [127:0] inv_mix_columns;
        ...
        begin
            for (i = 0; i < 4; i = i + 1) begin
                ...
                u = gf_mul2(gf_mul2(a0 ^ a2));
                v = gf_mul2(gf_mul2(a1 ^ a3));
                a0 = a0 ^ u;
                a1 = a1 ^ v;
                a2 = a2 ^ u;
                a3 = a3 ^ v;
                
                inv_mix_columns[i*32 +: 8] = gf_mul2(a0) ^ gf_mul3(a1) ^ a2 ^ a3;
                inv_mix_columns[i*32 + 8 +: 8] = a0 ^ gf_mul2(a1) ^ gf_mul3(a2) ^ a3;
                inv_mix_columns[i*32 + 16 +: 8] = a0 ^ a1 ^ gf_mul2(a2) ^ gf_mul3(a3);
                inv_mix_columns[i*32 + 24 +: 8] = gf_mul3(a0) ^ a1 ^ a2 ^ gf_mul2(a3);
            end
        end
    endfunction
    ...
    function [127:0] add_round_key;
        input [127:0] in;
        input [127:0] rkey;
        add_round_key = in ^ rkey;
    endfunction

    // Simplified key schedule (uses same key for demo)
    function [127:0] get_round_key;
        ...
        begin
            case(round_num)
                ...
                default: rcon = 8'h00;
            endcase
            // Simplified: XOR key with round constant
            get_round_key = key_in ^ {rcon, 120'h0};
        end
    endfunction
            ...
            if (start && !busy) begin
                ...
                if (encrypt) begin
                    ...
                    case(round)
                        ...
                        4'h1, 4'h2, 4'h3, 4'h4, 4'h5, 4'h6, 4'h7, 4'h8, 4'h9: begin
                            temp_state = sub_bytes(state);
                            temp_state = shift_rows(temp_state);
                            temp_state = mix_columns(temp_state);
                            round_key <= get_round_key(round_key, round);
                            state <= add_round_key(temp_state, get_round_key(round_key, round));
                            round <= round + 1;
                        end
                        4'hA: begin
                            temp_state = sub_bytes(state);
                            temp_state = shift_rows(temp_state);
                            state <= add_round_key(temp_state, get_round_key(round_key, round));
                            data_out <= add_round_key(temp_state, get_round_key(round_key, round));
                            done <= 1'b1;
                            busy <= 1'b0;
                            round <= 4'h0;
                        end
                    ...
                    // Decryption - simplified approach: use encryption with inverse operations
                    case(round)
                        4'h0: begin
                            state <= add_round_key(state, get_round_key(round_key, 4'hA));
                            round <= round + 1;
                        end
                        4'h1, 4'h2, 4'h3, 4'h4, 4'h5, 4'h6, 4'h7, 4'h8, 4'h9: begin
                            temp_state = inv_shift_rows(state);
                            temp_state = inv_sub_bytes(temp_state);
                            temp_state = add_round_key(temp_state, get_round_key(round_key, 4'hA - round));
                            state <= inv_mix_columns(temp_state);
                            round <= round + 1;
                        end
                        4'hA: begin
                            temp_state = inv_shift_rows(state);
                            temp_state = inv_sub_bytes(temp_state);
                            state <= add_round_key(temp_state, key);
                            data_out <= add_round_key(temp_state, key);
                            done <= 1'b1;
                            busy <= 1'b0;
                            round <= 4'h0;
                        end
